...
/** Удаляет устаревший кеш принятых количеств (больше не используется). */
export function purgeLegacyReceiptAcceptedHints() {
  try {
    sessionStorage.removeItem("sklad:receiptAcceptedHints");
  } catch {
    // ignore
  }
}
